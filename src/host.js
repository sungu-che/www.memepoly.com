...
function formSubmit(){
	...
	var res = function(res, selector){
		console.log("리턴 값",res);
	}

	OAuth3.fetch(req, res);
}
...
OAuth3.on("ready", function(){
	...
	var response = function(res){
		...
		if(address){
			document.querySelector(".aside .status").innerHTML = ''
		}else{
			document.querySelector(".aside .status").innerHTML = '<a href="/login/">Sign In</a>'
		}
		...
		var host_flag = "#tag "+window.location.host;
		...
		if(hash){
			host_flag = host_flag.replace("#", hash);
		}

		
		if(contenteditable){
			OAuth3.submit = function($form){
				var url = new URL($form.getAttribute("cc"));
				var Cc = url.pathname+url.search;
