					...
					if(stickers.length){
						for(var i = 0; i < stickers.length; i++){
							...
							if(len == row.index){
								...
								li += `<div id="${row.Id}" draggable="false" class="emoji_asset ${(isToggle ? "on" : "")} ${(row.new ? "new" : "")}" emoji="${emoji}" type="item"><a class="emoji color">${emoji}</a><span class="cnt">${cnt}</span></div>`	
							}
						}
					}
